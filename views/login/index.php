...
$active = ' class="active"';
?>
<?php if($this->loginMode!=="modal"){?><div class="container"><?php } ?>
    <div id="loginform">
        <!--//    make by Shikaru--> 
        <form class="form-signin" id="loginform_form" role="form" action="login/run" method="post">
        <?php if (sizeof($User) <= 0) { ?>  
            <h2 class="form-signin-heading">Please sign in</h2>
            <p>กรอกชื่อผู้ใช้และรหัสผ่าน เพื่อเข้าใช้งานระบบ <?=SHORT_NAME_SYSTEM?></p>
            <hr>
        ...
        <?php }else{ ?>
            <a href="<?= URL; ?>login/logout" class="btn btn-lg btn-warning btn-block">ออกจากระบบ</a>
        <?php } ?>
        </form>
    </div>
<?php if($this->loginMode!=="modal"){?></div><?php } ?>